...
double hera_x[52]={0.00003,0.00007,0.00013,0.00018,0.00034,0.00007,0.00018,0.00035,0.00099,0.00013,0.00018,0.00030,0.00049,0.00079,0.00158,0.00022,0.00031,0.00049,0.00079,0.00150,0.00301,0.00035,0.00049,0.00079,0.00134,0.00248,0.00458,0.00060,0.00079,0.00142,0.00241,0.00318,0.00541,0.00800,0.00138,0.00198,0.00318,0.00498,0.00800,0.01478,0.00198,0.00318,0.00556,0.01000,0.02512,0.00498,0.01286,0.01028,0.02512,0.01286,0.03140,0.04908};
double hera_y[52]={0.194,0.187,0.165,0.165,0.129,0.395,0.387,0.344,0.308,0.681,0.660,0.638,0.617,0.574,0.545,1.069,1.033,0.983,0.918,0.890,0.846,1.413,1.377,1.341,1.284,1.248,1.191,1.879,1.858,1.758,1.693,1.679,1.628,1.571,2.253,2.245,2.174,2.109,2.073,2.009,2.805,2.633,2.618,2.554,2.468,3.178,3.106,3.730,3.615,4.333,4.211,4.842};
...
void Check(){
    ...
    setXbins();
    setQ2bins();
    TFile *f1 = new TFile("./out_10_100_LANL.root","READ");
    TFile *f2 = new TFile("./out_10_100_LBNL.root","READ");
    h3d1 = (TH3F*) f1->Get("D0_RedCS_Topo_BKG");h3d1->SetName("h3d1");h3d1->SetDirectory(0);
    //h3d_bkg1 = (TH3F*) f1->Get("D0_RedCS_BKG");h3d_bkg1->SetName("h3d_bkg1");h3d_bkg1->SetDirectory(0);
    h3d_bkg1 = (TH3F*) f2->Get("D0_RedCS_Topo_BKG");h3d_bkg1->SetName("h3d_bkg1");h3d_bkg1->SetDirectory(0);
...
void CalcWithHera(int save=0){
    char outname[100]="eAu_output.root";
    char system[10]="eAu";
    gROOT->ProcessLine(".x ~/myStyle.C");
...
//============================
    //TFile *f1 = new TFile("./out_10_100_PID_New2.root","READ");
...
//============================  
    //TFile *f2 = new TFile("./out_5_41_PID_New2.root","READ");
...
//============================                      
    //TFile *f1_10 = new TFile("./out_10_100_Q210_PID_New2.root","READ");
...
//============================                                            
    //TFile *f2_10 = new TFile("./out_5_41_Q210_PID_New2.root","READ");
...
//============================                                                                                                                                                                                                                             
    TFile *f11 = new TFile("./out_10_100_LBNL_PID.root","READ");
...
//============================                                                                                                                                                                                                                        
    TFile *f22 = new TFile("./out_5_41_LBNL_PID.root","READ");
...
//============================  
    TFile *f11_10 = new TFile("./out_10_100_LBNL_Q210_PID.root","READ");
...
//============================ 
    TFile *f22_10 = new TFile("./out_5_41_LBNL_Q210_PID.root","READ");
...
*/
    ...
    for(int i = 0;i<nbins_q2-2;i++){
	    ...
	    if(y>0){
		double scale_ = (y + 0.04*TMath::Power((i+1),1.7))/y;
		y = y + 0.04*TMath::Power((i+1),1.7);
		err*=scale_;
    ...
    double hera_xlog[52];
    for(int i = 0;i<52;i++){
	hera_xlog[i]= log10(hera_x[i]);
	cout << hera_xlog[i] << " " << hera_x[i] << endl;
    }
TGraph *gF2_Hera = new TGraph(52,hera_xlog,hera_y);
    gF2_Hera->SetMarkerStyle(25);
    gF2_Hera->SetMarkerColor(1);    
    gF2->SetMarkerColor(kBlue);
    gF2->SetLineColor(kBlue);
    TCanvas *cf2 = new TCanvas("cf2","cf2",1200,1200); 
    gF2->GetXaxis()->SetTitle("log(x_{B})");
    gF2->GetYaxis()->SetTitle("F_{2}^{c#bar{c}}(x_{B},Q^{2}) #times C");
    gF2->GetYaxis()->SetTitleOffset(0.85);
    gF2->GetYaxis()->SetLimits(-5,5);
    gF2->GetYaxis()->SetRangeUser(-0.5,5);
    gF2->GetXaxis()->SetLimits(-6.9,0);
    gF2->GetXaxis()->SetRangeUser(-5.,0);
    gF2->Draw("APE");
    gF2_Hera->Draw("P same");
    TLegend *leg_f2 = new TLegend(0.15,0.7,0.4,0.9);
    leg_f2->SetTextSize(0.03);
    leg_f2->AddEntry(gF2,"EIC(#sqrt{s}=63,29 GeV) C = 0.04#it{i}^{1.7}","P");
    leg_f2->AddEntry(gF2_Hera,"H1+ZEUS C = 0.07#it{i}^{1.7}","P");
    leg_f2->Draw("same");
    ...
    double qsh[12]={2.5,5,7,12,18,32,60,120,200,540,650,2000};
	...
	if(i<3)lat.DrawLatex(latx1[i],i*0.5,Form("#color[4]{Q^{2}=%1.1f GeV^{2}}",qs[i]));
	else lat.DrawLatex(latx1[i],i*0.5,Form("#color[4]{Q^{2}=%1.f GeV^{2}}",qs[i]));
    }
    for(int i = 0;i<12;i++){
	if(i<1)lat.DrawLatex(-4.9,i*0.3,Form("#color[1]{Q^{2}=%1.1f GeV^{2}}",qsh[i]));
        else lat.DrawLatex(-4.9,i*0.3,Form("#color[1]{Q^{2}=%1.f GeV^{2}}",qsh[i]));
    }
    lat.SetTextAngle(0);
    //lat.DrawLatex(-0.8,0.1,"#color[4]{Q^{2} (GeV^{2}), C}");
    TF1 *lline = new TF1("lline","0",-100,100);
    lline->Draw("same");
