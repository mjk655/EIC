...
void PlotSepGluon(int save=0){
    ...
    TFile *fin1 = new TFile("CTEQ_Gluon_NewWeights.root","READ");
    for(int i = 1;i<nbins_q2-2;i++){
    ...
    TF1 *line1 = new TF1("line1","0.1",-100,100);
	...
	Base_EPPS[bin]->GetXaxis()->SetTitle("log_{10}(x)");
	...
	if(i==1){
	    TLegend *canv = new TLegend(0.31,0.57,0.51,0.87);
	...
	if(i==2){
            TLegend *canv = new TLegend(0.1,0.57,0.35,0.87);
            canv->SetTextSize(0.055);
	...
	ERRS_EPPS[bin]->DrawClone("hist ][");
        if(i==1)ERRS_New_EPPS[bin]->DrawClone("P same ");
	if(i==2)ERRS_CTEQ[bin]->DrawClone("same hist ][");
        if(i==2)ERRS_New_CTEQ[bin]->DrawClone("P same ");
	if(i==3)ERRS_NNPDF[bin]->DrawClone("same hist ][");
        if(i==3)ERRS_New_NNPDF[bin]->DrawClone("P same ");
