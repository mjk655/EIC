...
void Plot_EPPS(int save=0){
    ...
    TFile *fin = new TFile("results11_NewWeights_noT_highstat.root","READ");
...
/*           
    ...
    if(save)TFile *fout = new TFile("EPPS_Gluon_NewWeights_noT.root","RECREATE");
    //for(int i = 1;i < 3; i++){
	for(int i = 1;i < nbins_q2-2; i++){   
	c111->cd(i);
...
/*
	...
	lat.DrawLatex(-3.7,1.7,Form("Q^{2} = %1.f GeV^{2}",qq[i])); 
       ERRS_W[i]->Write(Form("ERRS_EPPS_W_%i",i+1));
            ERRS[i]->Write(Form("ERRS_EPPS_%i",i+1));
	    }
*/
