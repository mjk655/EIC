...
void Plot_CTEQ(int save=1){
    ...
    TFile *fin = new TFile("results3_NewWeights_noT.root","READ");
...
*/          
    ...
    TCanvas *c11 = new TCanvas("c11","f2",1500,700);
    // Canvas setup                                                                                                                                                                                                                                    
    const Int_t Nx1 = 5;
    const Int_t Ny1 = 2;
    ...
    if(save)TFile *fout = new TFile("CTEQ_Gluon_NewWeights_noT.root","RECREATE");
	...
	TH1F* True = (TH1F*)N_GLUON[0]->ProjectionX("True",i+1,i+1);
        TH1F* Mod = (TH1F*)N_GLUON_Weight[0]->ProjectionX("Mod",i+1,i+1);
        TH1F* Ini = (TH1F*)N_GLUON_Comb[0]->ProjectionX("Ini",i+1,i+1);
	...
	for(int ii = 1; ii< Ini->GetNbinsX()+1; ii++){
	    ...
	    if(sum2>0){
		ERRS_W[i]->SetBinContent(ii,sqrt(sum22)/sum2);
	...
	lat.SetTextSize(0.08);
	lat.DrawLatex(-1.7,1.7,Form("Q^{2} = %1.f GeV^{2}",qq[i-1]));
	gPad->RedrawAxis();
	if(save){
	Ini->Write(Form("Base_CTEQ_%i",i));
        Mod->Write(Form("New_CTEQ_%i",i));
        ERRS_W[i]->Write(Form("ERRS_New_CTEQ_%i",i));
        ERRS[i]->Write(Form("ERRS_CTEQ_%i",i));
	}
    }
