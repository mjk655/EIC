...
//============================  
    ...
    TFile *f2 = new TFile("./out_10_100_fulltree_nucl.root","READ");
    h3d2 = (TH3F*) f2->Get("D0_RedCS_T");h3d2->SetName("h3d2");h3d2->SetDirectory(0);
    h3d_bkg2 = (TH3F*) f2->Get("D0_RedCS_BKG");h3d_bkg2->SetName("h3d_bkg2");h3d_bkg2->SetDirectory(0);
...
//============================                                            
    ...
    TFile *f2_10 = new TFile("./out_10_100_fulltree_nucl_Q210.root","READ");
    h3d2_10 = (TH3F*) f2_10->Get("D0_RedCS_T");h3d2_10->SetName("h3d2_10");h3d2_10->SetDirectory(0);
    h3d_bkg2_10 = (TH3F*) f2_10->Get("D0_RedCS_BKG");h3d_bkg2_10->SetName("h3d_bkg2_10");h3d_bkg2_10->SetDirectory(0);
    ...
    CS2_10 = getXsec(_hxQ22_10,1/0.042);
    cout << "Calculated Lumi in 5x41 q2=10 is " << CS2_10 << endl;
    
    setXbins();
    setQ2bins();
    
    TH2F *hCS_x1 = new TH2F("hCS_x1","hCS_x1",nbins_x,binning_x,nbins_q2,binning_q2);
    TH2F *hCS_x2 = new TH2F("hCS_x2","hCS_x2",nbins_x,binning_x,nbins_q2,binning_q2);
    ...
    for(int i = 0;i<nbins_q2;i++){
	if(i<5){
	    qs1[i] = getRCSVsX(hmean_q21[i],hmean_x1[i],hCS_x1,hdCS_x1[0],h3d1,h3d_d1,h3d_bkg1,i+1,CS1,2,10);
	    qs2[i] = getRCSVsX(hmean_q22[i],hmean_x2[i],hCS_x2,hdCS_x2[0],h3d2,h3d_d2,h3d_bkg2,i+1,CS2,3,10);
	}
	else{
	    qs1[i] = getRCSVsX(hmean_q21[i],hmean_x1[i],hCS_x1,hdCS_x1[0],h3d1_10,h3d_d1_10,h3d_bkg1_10,i+1,CS1_10,2,10);
            qs2[i] = getRCSVsX(hmean_q22[i],hmean_x2[i],hCS_x2,hdCS_x2[0],h3d2_10,h3d_d2_10,h3d_bkg2_10,i+1,CS2_10,3,10);
	}
    }
	...
	for(int j = 1;j<nbins_q2;j++){
	    cout << j  << " " << i << " " << hCS_x1->GetBinContent(i,j) << " " << hCS_x2->GetBinContent(i,j) << endl;
	}
    }
    hCS_x1->Divide(hCS_x2);
    hCS_x1->GetZaxis()->SetRangeUser(0,3);
    hCS_x1->Draw("COLZ");
    gPad->SetRightMargin(0.1);

}
double getRCSVsX(TH1F *means_q2, TH1F *means_x, TH2F * fill, TH1F *dfill,TH3F *h3d,TH3F *h3d_d,TH3F *h3d_bkg,int qbin, double xsec, int ss, double L){
...
//	h3d->GetZaxis()->SetRangeUser(1.855,1.875);
//	h3d_bkg->GetZaxis()->SetRangeUser(1.855,1.875);
...
//continue;
	...
	fill->SetBinContent(i,qbin,up*val*scale/ (sqrt(up)*scale*sqrt(val+val2)));
	fill->SetBinError(i,qbin,0);//sqrt(up)*scale*sqrt(val+val2));
