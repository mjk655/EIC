...
void PlotGluon(int save=1){
    ...
    TFile *fin = new TFile("EPPS_Gluon.root","READ");
    for(int i = 1;i<nbins_q2-2;i++){
	ERRS_New_EPPS[i] = (TH1D*)fin->Get(Form("ERRS_New_EPPS_%i",i));
    ...
    TFile *fin1 = new TFile("CTEQ_Gluon.root","READ");
    for(int i = 1;i<nbins_q2-2;i++){
    ...
    TFile *fin2 = new TFile("NNPDF_Gluon.root","READ");
    for(int i = 1;i<nbins_q2-2;i++){
    ...
    TCanvas *c111 = new TCanvas("c111","gluon ratios",1300,700);
    // Canvas setup
    const Int_t Nx = 3;
    const Int_t Ny = 2;
	...
	Base_EPPS[bin]->GetXaxis()->SetTitle("log(x)");
	Base_EPPS[bin]->GetYaxis()->SetTitleSize(0.14);
	Base_EPPS[bin]->GetYaxis()->SetTitleOffset(0.7);
	Base_EPPS[bin]->GetYaxis()->SetLabelSize(0.08);
	...
	if(i==1){
	    TLegend *canv = new TLegend(0.5,0.57,0.69,0.87);
	    canv->SetTextSize(0.055);
	    canv->SetHeader("EPPS16");
	    canv->AddEntry(temp1,"Baseline","f");
	    canv->AddEntry(New_EPPS[bin],"With F_{2}^{c#bar{c}}","fl");
	}
	if(i==2){
            TLegend *canv = new TLegend(0.1,0.57,0.45,0.87);
            canv->SetTextSize(0.055);
	...
	temp1->DrawClone("E2 same");
	temp2->DrawClone("E2 same");
	temp3->DrawClone("E2 same");

	Base_EPPS[bin]->DrawClone("hist same ][");
	New_EPPS[bin]->DrawClone("E2 same");
	New_EPPS[bin]->SetFillColor(0);
	New_EPPS[bin]->DrawClone("hist same ][");
	Base_CTEQ[bin]->DrawClone("hist same ][");
        New_CTEQ[bin]->DrawClone("E2 same");
	New_CTEQ[bin]->SetFillColor(0);
	New_CTEQ[bin]->DrawClone("Hist same ][");
	Base_NNPDF[bin]->DrawClone("hist same ][");
        New_NNPDF[bin]->DrawClone("E2 same");
	New_NNPDF[bin]->SetFillColor(0);
	New_NNPDF[bin]->DrawClone("hist same ][");
	New_EPPS[bin]->SetFillColor(15);
	New_CTEQ[bin]->SetFillColor(38);
	New_NNPDF[bin]->SetFillColor(45);
	...
	lat.DrawLatex(-1.7,1.9,Form("Q^{2} = %1.f GeV^{2}",qq[bin]));
	gPad->RedrawAxis();
	pad[i-1][0]->cd();
	...
	ERRS_EPPS[bin]->GetYaxis()->SetTitle("Relative Uncertainty  ");                                                                                                                                                                           
        ERRS_EPPS[bin]->GetYaxis()->SetRangeUser(0.005,3.1);                                                                                                                                                                                               
        ERRS_EPPS[bin]->GetXaxis()->SetTitle("log(x)");
	ERRS_EPPS[bin]->GetYaxis()->SetLabelSize(0.08);
	ERRS_EPPS[bin]->GetXaxis()->SetLabelSize(0.08);
	ERRS_EPPS[bin]->GetXaxis()->SetTitleSize(0.1);
	ERRS_EPPS[bin]->GetYaxis()->SetTitleSize(0.1);
	...
	c111->SaveAs("paper/GluonRatios.ps");
    }
