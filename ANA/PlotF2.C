...
void PlotF2(int save=0){
    ...
    TFile *fin3 = new TFile("ep_output_new.root","READ");
    ...
    double latx[nbins_q2-2]={-2.79,-2.8,-2.6,-2.3,-2.1,-1.95,-1.75,-1.55,-1.35,-1.2,-1.1};
    double laty[nbins_q2-2]={0.0007,0.015,0.2,2,13,100,500,2000,9000,50000,200000};
    ...
    gF2->GetYaxis()->SetRangeUser(0.02,80000000);
    ...
    TLegend *leg = new TLegend(0.23,0.85,0.4,0.92);
    leg->SetTextSize(0.032);
    
    leg->SetHeader("PYTHIA e+p #sqrt{s}=63,29 GeV, #it{L}=10 fb^{-1}");
    //leg->AddEntry(gF2,"e+p Projection","PE");
    //leg->AddEntry(hF2_x_IC1[1],"e_Au Projection","l");
    leg->Draw("same");
    lat.SetTextSize(0.03);
    lat.DrawLatex(-2.8,1000000,"#color[4]{Q^{2} (GeV/#it{c}^{2})[i]}");
    lat.DrawLatex(-2.8,200000,"#color[4]{C=7^{i}}");
