...
void DoAna(int index=1){
    bool VTX_FIT = false;
    bool onlyD0 = true;
    cout <<"Now doing DoAna.C " << endl;
    ...
    TFile *partonf = new TFile("PartonWeights_IC_NNPDF.root","READ");
    ...
    TFile *vtxres = new TFile("VertexRes_v2_p6_New.root","READ");
    TH1F* VertexRes_X = (TH1F*) vtxres->Get("hx");VertexRes_X->SetName("VertexRes_X");
    TH1F* VertexRes_Y = (TH1F*) vtxres->Get("hy");VertexRes_Y->SetName("VertexRes_Y");
    TH1F* VertexRes_Z = (TH1F*) vtxres->Get("hz");VertexRes_Z->SetName("VertexRes_Z");
    ...
    TFile *trackres = new TFile("MomResolution_v2_p8.root","READ");
    TH2F* MomResEtaVsP = (TH2F*) trackres->Get("MomResEtaVsP");
    MomResEtaVsP->SetDirectory(0);
    TFile *trackres_k = new TFile("MomResolution_Kaon_v2_p8.root","READ");
    TH2F* MomResEtaVsP_K = (TH2F*) trackres_k->Get("MomResEtaVsP_K");
    MomResEtaVsP_K->SetDirectory(0);

    TH1F *hDCATRes_Plus[38][3];
    TH1F *hDCATRes_Minus[38][3];

    cout << "Getting DCA Res " << endl;
    TFile *trackdca = new TFile("DCAResolution_v2_p8.root","READ");
    cout <<"Reading res histos " << endl;
    TH1F* DCARes_T_Eta1 = (TH1F*) trackdca->Get("DCARes_T_Eta1");DCARes_T_Eta1->SetDirectory(0);
    TH1F* DCARes_T_Eta2 = (TH1F*) trackdca->Get("DCARes_T_Eta2");DCARes_T_Eta2->SetDirectory(0);
    TH1F* DCARes_T_Eta3 = (TH1F*) trackdca->Get("DCARes_T_Eta3");DCARes_T_Eta3->SetDirectory(0);
    TH1F* DCARes_Z_Eta1 = (TH1F*) trackdca->Get("DCARes_Z_Eta1");DCARes_Z_Eta1->SetDirectory(0);
    TH1F* DCARes_Z_Eta2 = (TH1F*) trackdca->Get("DCARes_Z_Eta2");DCARes_Z_Eta2->SetDirectory(0);
    TH1F* DCARes_Z_Eta3 = (TH1F*) trackdca->Get("DCARes_Z_Eta3");DCARes_Z_Eta3->SetDirectory(0);
    
    cout << "Getting DCA histograms " << endl;
    for(int i = 0;i<38;i++){
	for(int j = 0;j<3;j++){
	    char label[100];
	    sprintf(label,"hDCATRes_Plus_%i_%i",i,j+1);
	    hDCATRes_Plus[i][j] = (TH1F*)trackdca->Get(label);hDCATRes_Plus[i][j]->SetDirectory(0);
	    sprintf(label,"hDCATRes_Minus_%i_%i",i,j+1);
            hDCATRes_Minus[i][j] = (TH1F*)trackdca->Get(label);hDCATRes_Minus[i][j]->SetDirectory(0);
	}
    }
...
/*	EICVertexFitter *pVtx = new EICVertexFitter("pVtx");
	...
	for(int tr = 0; tr<mntracks ; tr++){
		...
		if(fabs(TMath::ATanH(mpz[tr]/p))<3)hf_flag3=1;
	    }
		...
		if(fabs(TMath::ATanH(mpz[tr]/p))<1){
		    ...
		    if(!passTracking(TrackEff_EtaP,pt,(fabs(TMath::ATanH(mpz[tr]/p)))))mult++;
		}
		if(fabs(TMath::ATanH(mpz[tr]/p))<3){
		    ...
		    if(passTracking(TrackEff_EtaP,pt,(fabs(TMath::ATanH(mpz[tr]/p)))))
			mult3++;
		}
	    }
	}
...
/*	if(VTX_FIT){
	    ...
	    hVtx_Mult_XY->Fill(mult,xyres);
	    hVtx_Mult_Z->Fill(mult,vertex.Z());
	...
	double newx = gRandom->Gaus(0,0.010);//VertexRes_X->GetBinContent(VertexRes_X->FindBin(mult3+0.1)))/1000.;
	double newy = gRandom->Gaus(0,0.010);//VertexRes_Y->GetBinContent(VertexRes_Y->FindBin(mult3+0.1)))/1000.;
        double newz = gRandom->Gaus(0,0.013);//VertexRes_Z->GetBinContent(VertexRes_Z->FindBin(mult3+0.1)))/1000.;
...
//if(theta<0)theta+=TMath::Pi()*2.;                                                                                                                                                                                                                                     //theta = theta*180./TMath::Pi();                                                                                                                                                                                                                       
		...
		TLorentzVector *p1 = new TLorentzVector(mpx[tr],mpy[tr],mpz[tr],me[tr]);
			...
			if(!passTracking(TrackEff_EtaP,p1->Pt(),fabs(p1->PseudoRapidity())) || !passTracking(TrackEff_EtaP,p2->Pt(),fabs(p2->PseudoRapidity()))){
			    delete p2; delete ps2; delete Ps;
			    continue;
			}
			if(bkg_flag==1 && fabs(Ps->PseudoRapidity())<3 && fabs(Ps->M()-1.86484)<0.02)D0_RedCS_BKG->Fill(log10(mx),log10(mq2),Ps->M());
			...
			if(isGoodD0_XY(Ps,ps1->Vect(),ps1_vtx,ps2->Vect(),ps2_vtx,vertex,fabs(mid[tr]),fabs(mid[tr1]),TrackEff_EtaP,dofill,ps1->E(),ps2->E())){
			    if(bkg_flag==1 && fabs(Ps->PseudoRapidity())<3 && fabs(Ps->M()-1.86484)<0.02){
				check1=1;
				double dphi = Ps->Phi();
...
//if(isFromB(gggmotherID))cout << mid[tr] << " mother " << motherID << "  " << gmotherID << " " << ggmotherID << " " << gggmotherID << " " << ggggmotherID << " " << ggggmotherID <<" " << gggggmotherID<<endl; 
		...
		if(num==1 && (fabs(mid[tr]) == 421||fabs(mid[tr]) == 443)){//Two-body decays
		    ...
		    if(fabs(Ps->PseudoRapidity())>5 || !passTracking(TrackEff_EtaP,p1->Pt(),fabs(p1->PseudoRapidity())) || !passTracking(TrackEff_EtaP,p2->Pt(),fabs(p2->PseudoRapidity()))){
			...
			if(fabs(Ps->PseudoRapidity())<3)D0_RedCS_T->Fill(log10(mx),log10(mq2),Ps->M());
		    }
		    //TVector3 ps1_vtx = smearPosEIC2(ps1,p1_vtx,DCARes_T_Eta1,DCARes_T_Eta2,DCARes_T_Eta3,DCARes_Z_Eta1,DCARes_Z_Eta2,DCARes_Z_Eta3);
		    ...
		    //TVector3 ps1_vtx = smearPosEIC3(ps1,p1_vtx,hDCATRes_Plus[getPtIdx(ps1->Pt())][getEtaIdx(ps1->PseudoRapidity())],hDCATRes_Minus[getPtIdx(ps1->Pt())][getEtaIdx(ps1->PseudoRapidity())],DCARes_Z_Eta1,DCARes_Z_Eta2,DCARes_Z_Eta3,mid[mfd[tr]-1]);
                    //TVector3 ps2_vtx = smearPosEIC3(ps2,p2_vtx,hDCATRes_Plus[getPtIdx(ps2->Pt())][getEtaIdx(ps2->PseudoRapidity())],hDCATRes_Minus[getPtIdx(ps2->Pt())][getEtaIdx(ps2->PseudoRapidity())],DCARes_Z_Eta1,DCARes_Z_Eta2,DCARes_Z_Eta3,mid[mld[tr]-1]);
		    TVector3 ps1_vtx = smearPosEIC(ps1,p1_vtx);      
		    TVector3 ps2_vtx = smearPosEIC(ps2,p2_vtx);      
		    TVector3 Ps_vtx = (ps1_vtx+ps2_vtx)*0.5;
		    double _dca = dcaXY(Ps->Vect(), Ps_vtx, vertex,mult3);
			...
			if(isKPiDecay(mid[mfd[tr]-1],mid[mld[tr]-1])){
			    ...
			    if(D0flag){
				if(fabs(Ps->PseudoRapidity())<3 && fabs(Ps->M()-1.86484)<0.02){
				    check2=1;
				    double dphi = Ps->Phi();
				    if(dphi<0)dphi+=TMath::Pi()*2.;
				    double delta_phi = fabs(dphi - mphi)/TMath::Pi();
