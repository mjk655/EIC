...
void InclCalc(int save=1){
    ...
    double CS22_10 = 1;
//============================
    TFile *f1 = new TFile("./out_10_100_Incl.root","READ");
    h3d1 = (TH3F*) f1->Get("Incl_RedCS");h3d1->SetName("h3d1");h3d1->SetDirectory(0);
    TH2F* _hxQ21 = (TH2F*) f1->Get("hxQ2");_hxQ21->SetName("_hxQ21");
    CS1 = getXsec(_hxQ21,1/0.65);
    cout << "Calculated Lumi in 10x100 is " << CS1 << endl;
//============================  
    TFile *f2 = new TFile("./out_5_41_Incl.root","READ");
    h3d2 = (TH3F*) f2->Get("Incl_RedCS");h3d2->SetName("h3d2");h3d2->SetDirectory(0);
...
//============================                                                                                                                                                                                                               
    ...
    double qs22[20];
    for(int i = 0;i<nbins_q2;i++){
	qs1[i] = getRCSVsX(hmean_q21[i],hmean_x1[i],hCS_x1[i],hdCS_x1[i],h3d1,i+1,CS1,2,10);
	qs2[i] = getRCSVsX(hmean_q22[i],hmean_x2[i],hCS_x2[i],hdCS_x2[i],h3d2,i+1,CS2,3,10);
    }
    ...
    double qs_2[20];
    for(int i = 0;i<nbins_q2;i++){
        qs_1[i]  = getF2VsX(hF2_x[i],hdF2_x[i],hsdF2_x[i],hs1dF2_x[i],hCS_x1[i],hmean_x1[i],hmean_q21[i],hCS_x1[i],hmean_x1[i],hmean_q21[i],hCS_x2[i],hmean_x2[i],hmean_q22[i],i+1,cfit1);
	qs_2[i]  = getF2VsX(hF2_x2[i],hdF2_x2[i],hsdF2_x2[i],hs1dF2_x2[i],hCS_x1[i],hmean_x1[i],hmean_q21[i],hCS_x1[i],hmean_x1[i],hmean_q21[i],hCS_x2[i],hmean_x2[i],hmean_q22[i],i+1,cfit2);
    }
	...
	leg1->AddEntry(hCS_x1[i],label,"l");
	leg2->AddEntry(hCS_x1[i],label,"l");
	sprintf(label,"Q^{2} = %1.1f GeV^{2}, C = %1.3f ",qs[i],scale[i]);
	if(i<6){
	    sprintf(label,"Q^{2} = %1.1f GeV^{2}, C = %1.3f ",qs[i],scale[i]);
	    leg->AddEntry(hCS_x1[i],label,"PE");
	}        
	else {
	    sprintf(label,"Q^{2} = %1.1f GeV^{2}, C = %1.1f ",qs[i],scale[i]);
	    leg_2->AddEntry(hCS_x1[i],label,"PE");
	}
	if(i<6){
	    sprintf(label,"Q^{2} = %1.1f GeV^{2}, C = %1.3f ",qs[i],scale[i]);
	    legF2->AddEntry(hCS_x1[i],label,"PE");
	}
	else {
	    sprintf(label,"Q^{2} = %1.1f GeV^{2}, C = %1.1f ",qs[i],scale[i]);
	    legF22->AddEntry(hCS_x1[i],label,"PE");
	}
    }
...
//=================== All in one TGraph =====
    ...
    gCS1->GetYaxis()->SetTitle("#sigma_{r}/(dQ^{2}dx) #times C (mb)");
    gCS1->GetYaxis()->SetLimits(0.00000001,5);
    gCS1->GetYaxis()->SetRangeUser(0.0000005,0.4);
...
//	hCS_x2[i]->Draw("E5 same");
    ...
    leg_cs->AddEntry(gCS1,"10+100 GeV 10 fb^{-1}","P");
    leg_cs->AddEntry(gCS2,"5+41 GeV 10 fb^{-1} (x1.2)","P");
...
//    TCanvas *cf2 = new TCanvas("cf2","cf2");
    ...
    hF2_x[0]->GetYaxis()->SetRangeUser(0.00001,0.1);//hF2_x[0]->GetMaximum()*5);
    for(int i = 0;i<nbins_q2-2;i++){
	hF2_x[i]->Scale(scale[i]);
	hF2_x[i]->GetYaxis()->SetRangeUser(0.00001,0.1);
...
*/
    ...
    gF2->GetYaxis()->SetTitle("F_{2} #times C");
    gF2->GetYaxis()->SetLimits(0.0000001,5);
    gF2->GetYaxis()->SetRangeUser(0.000005,0.2);
...
*/    
    ...
    hdF2_x[0]->GetYaxis()->SetRangeUser(0.01,1);        
...
double getRCSVsX(TH1F *means_q2, TH1F *means_x, TH1F * fill, TH1F *dfill,TH3F *h3d,int qbin, double xsec, int ss, double L){
	...
	if(val/sqrt(val)<1)continue;
	double s = 0;
	if(ss==1)s = 4.*18.*275.;
	if(ss==2)s = 4.*10.*100.;
	if(ss==3)s = 4.*5.*41.;
        double mean_y = mean_q2/s/mean_x;
	double conv = 1./0.565577;//0.3894/0.565577;
	double factor = mean_x * TMath::Power(mean_q2,2)/(2*3.141592/137.06/137.06*(1+(1-mean_y)*(1-mean_y)));
        double scale = 0.5 * conv * 1./lumi * 1./fabs(TMath::Power(10,binning_x[i])-TMath::Power(10,binning_x[i-1])) * 1./width * factor;
	fill->SetBinContent(i,up*val*scale);
	fill->SetBinError(i,sqrt(up)*scale*sqrt(val));
	dfill->SetBinContent(i,((sqrt(up)*sqrt(val))/(up*val)));
    ...
    fill->GetYaxis()->SetTitle("#sigma_{r}/(dQ^{2}dx) #times C (mb)");
    fill->GetYaxis()->SetNdivisions(505);
    dfill->GetYaxis()->SetTitle("Relative Error on #sigma_{r}");//S/#sqrt{S+B}");
...
double getF2VsX(TH1F * fill,TH1F * dfill, TH1F * dsfill,TH1F * ds1fill,TH1F *cs1,TH1F *mean_x1, TH1F* mean_q21,  TH1F *cs2,TH1F *mean_x2, TH1F* mean_q22,  TH1F *cs3,TH1F *mean_x3, TH1F* mean_q23,int qbin,TCanvas *cc){
	...
	*/  
	...
	gr.GetYaxis()->SetTitle("#sigma_{r}(Q^{2},x) [mb]");
...
//    leggg->Draw("same");
    ...
    fill->GetXaxis()->SetRangeUser(-4,0);
    fill->GetXaxis()->SetTitle("log(x_{B})");
    fill->GetYaxis()->SetTitle("F_{2} #times C");
    fill->GetYaxis()->SetNdivisions(505);
    dfill->GetXaxis()->SetRangeUser(-4,0);
    dfill->GetXaxis()->SetTitle("log(x_{B})");
    dfill->GetYaxis()->SetTitle("#sigma(F_{2})/F_{2} (Stat.)");
